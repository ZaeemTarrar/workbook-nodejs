export {};
import { BAD_REQUEST, BAD_GATEWAY, FORBIDDEN, OK, UNAUTHORIZED, NOT_MODIFIED } from 'http-status';
...
// const { C_IP, C_PORT }: any = require('./../configurations/index');
// const SendEmail: any = require('./../utils/email');
...
scheme.methods.Tokenize = function(): any {
	return jwt.sign({ _id: this._id, contact: this.contact, email: this.email, password: this.password }, Jwt.Key, {
...
scheme.statics.Register = async function(data: any): Promise<any> {
	...
	return {
		creation: await creation.populate('SubRole', '-Users -Authorization -Role').execPopulate(),
...
// scheme.statics.RefreshToken = async function(res: any, token: string): Promise<any> {
// 	const user: any = await this.findOne({ token });
// 	if (!user) return R2(res, BAD_REQUEST, 'Invalid Token', null, 1);
// 	else {
// 		const session: any = await getSession(user._id);
// 		const sessionStatus: boolean = session === token ? true : false;
// 		if (!sessionStatus) return R2(res, UNAUTHORIZED, 'Session has been Eliminated, User has logged out', null, 1);
// 		else {
// 			await jwt.verify(token, JWT_KEY, (err: any, decoded: any): void => {
// 				(async (): Promise<any> => {
// 					if (!err && decoded) return R2(res, BAD_REQUEST, 'Token Not Expired', null, 2);
// 					else if (err && user) {
...
// 							);
// 							if (updated) {
// 								return R2(res, OK, 'Token Refreshed Successfully', { token: newToken });
// 							} else {
...
// 	}
// };
// scheme.statics.PasswordReset = async function(res: any, data: any, cb: Function): Promise<void> {
// 	if (!data.resetToken) return R2(res, BAD_REQUEST, 'Reset Token Not Provided', null, 1);
// 	else if (!data.password) return R2(res, BAD_REQUEST, 'Password Not Provided', null, 1);
// 	else {
// 		const user: any = await this.findOne({
// 			resetToken: data.resetToken
// 		}).exec();
// 		if (!user) return R2(res, BAD_REQUEST, 'Invalid Reset Token', null, 0);
// 		else {
// 			try {
// 				const decoded: any = await jwt.verify(data.resetToken, JWT_KEY);
// 				if (decoded) {
...
// 					};
// 					await this.findByIdAndUpdate(user._id, newData, {
// 						new: true,
// 						useFindAndModify: false
// 					});
// 					if (cb) cb();
// 				} else return R2(res, BAD_REQUEST, 'Reset Token is Expired', null, 1);
// 			} catch (e) {
// 				const newData = {
// 					resetToken: undefined,
// 					resetTokenExpiry: undefined
...
// 					useFindAndModify: false
// 				});
// 				return R2(res, BAD_REQUEST, `Reset Token is Expired: ${e.message}`, null, 1);
// 			}
// 		}
// 	}
// };
