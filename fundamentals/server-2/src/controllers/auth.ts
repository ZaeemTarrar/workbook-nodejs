...
const {
  ...
  WARNING,
  INFO,
}: any = require('./../static/Message/index')
...
module.exports.register = async (
  ...
  try {
    ...
    ApiTools.set(CREATED, [SUCCESS, 'User Regsitered'], { registeration })
      .show()
      .send()
  } catch (err) {
...
module.exports.login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!result.token)
      ApiTools.set(CREATED, [WARNING, 'Token cound not be created'], result)
        .show()
        .send()
    ...
    if ((<Error | any>err).code === 'ERR_ASSERTION')
      ApiTools.set(BAD_REQUEST, [WARNING, (<Error>err).message], null)
        .show()
        .send()
    else if ((<Error | any>err).code === 'jwt_error')
      ApiTools.set(BAD_REQUEST, [WARNING, (<Error>err).message], null)
        .show()
        .send()
    else
      ApiTools.set(BAD_REQUEST, [ERROR, (<Error>err).message], <Error>err)
        .show()
        .send()
  }
}
 ...
 */
module.exports.forgetPassword = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    assert(
      req.body.email,
      'User `email` i.e. ( email, contact ) is not provided',
    )
    ...
    events.on('email-sent', (status: boolean) => {
      if (!status)
        ApiTools.set(BAD_REQUEST, [WARNING, 'Email could not be sent'], result)
          .show()
          .send()
      else
        ApiTools.set(
          OK,
          [SUCCESS, 'Email to Reset Password has been Successfull sent'],
          {
            updation: result,
            resetUrl,
...
module.exports.resetPassword = async (
  ...
  try {
    ...
    assert(resetToken, 'User `resetToken` is not provided')
    assert(password, 'User new `password` is not provided')
    const result: any = await UsersCollection.ResetPassword(req.body)
    if (!result.updation)
      ApiTools.set(CREATED, [WARNING, 'Password could not be updated'], result)
        .show()
        .send()
    else
      ApiTools.set(OK, [SUCCESS, 'Password successfully updated'], result)
        .show()
        .send()
  } catch (err) {
    if ((<Error | any>err).code === 'ERR_ASSERTION')
