...
module.exports.fetchAll = async (
  ...
  try {
    ...
    if (subroles.length == 0)
      ApiTools.set(OK, [INFO, 'Empty List of SubRoles'], { subroles })
        .show()
        .send()
    else
      ApiTools.set(OK, [SUCCESS, 'List of All SubRoles'], { subroles })
        .show()
        .send()
  } catch (err) {
...
module.exports.fetchOne = async (
  ...
  try {
    ...
    if (!subrole)
      ApiTools.set(OK, [INFO, 'No SubRole Found'], { subrole }).show().send()
...
module.exports.create = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    ApiTools.set(CREATED, [SUCCESS, 'New SubRole Created'], result)
      .show()
      .send()
  } catch (err) {
    if ((<Error | any>err).code === 'ERR_ASSERTION')
...
module.exports.update = async (req: Request, res: Response): Promise<void> => {
  try {
    assert(req.params.id, 'SubRole `id` is not provided')
    const result: any = await SubRolesCollection.UpdateOne(
      req.params.id,
      req.body,
    )
