...
const { PORT, IP, CreateDb, RefreshDb, SeedDb, RemoveDb, ClearLogsOnEveryStartup } = require('./configurations/index');
	...
	.then((server: any): void => {
		server.listen(PORT, (): void => {
			...
			console.log(
				...
				bold(gray(`\t ( Server Execution Time: ${green(TimeTaken.toString() + ` seconds`)} )`))
			);
		});
	})
	.catch((err: Error) => {
