...
module.exports.Seeder = (): Promise<void> | void => {
     ...
     */
		...
		const users_hash_passwords: Promise<void> = Promise.all([ ...users_password_promises ]).then((result: any) => {
			for (let i in users) {
     ...
     */
		const Pivot = (
			...
			try {
				...
				console.log(bold(red(`Pivot Seeding Error: (${Table1}: ${id1}) (${Table2}: ${id2})`)));
			}
		};
     ...
     */
        ...
        */
				...
				let Users_Promise: Promise<void> = User.bulkCreate(users).then((result: any): any => {
					console.log(bold(cyan(`${blue(`User`)} Table Seeded`)));
					return result;
				});
				let Profiles_Promise: Promise<void> = Profile.bulkCreate(profiles).then((result: any): any => {
					console.log(bold(cyan(`${blue(`Profile`)} Table Seeded`)));
					return result;
				});
				let Categories_Promise: Promise<void> = Category.bulkCreate(categories).then((result: any): any => {
					console.log(bold(cyan(`${blue(`Category`)} Table Seeded`)));
					return result;
				});
				let Tags_Promise: Promise<void> = Tag.bulkCreate(tags).then((result: any): any => {
					console.log(bold(cyan(`${blue(`Tag`)} Table Seeded`)));
					return result;
				});
				let Products_Promise: Promise<void> = Product.bulkCreate(products).then((result: any): any => {
          ...
          */
           ...
           */
					...
					.catch((err: Error): void => {
						console.log(red('Database Tables Seeding Error: '), gray(err.message));
					});
			})
			.catch((e: Error) => {
